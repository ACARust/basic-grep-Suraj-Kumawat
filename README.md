...
The second set of tests (more complicated) could be run by the following command:

```bash
$ cargo test --test grep
```

Your aim is to atleast pass all the easy test cases upon making a submission.
...
It's okay to submit an incomplete solution. Do not blindly copy from the internet though, we know where the problem has been taken from, easy to check plagiarism if you don't know that yet.
